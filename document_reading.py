...
def load(name, path):
    ...
    print("Loading the object contained in obj/" + name + ".pkl \n")
    try:
...
def save(name, content, path):
    ...
    if os.path.exists("obj/{}/{}.pkl".format(path, name)):
        choice = str(input("Do you want to overwrite the existing file (Y/N) ?: \n"))
        if choice == "Y":
            ...
            new_name = input("Enter a new filename if you want to save, and 'N' to skip saving : ")
            if new_name != 'N':
